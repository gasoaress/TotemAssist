...
# Referenciar o elemento <body> da página
imagem = driver.find_element(By.TAG_NAME, 'body')

# Tirar print apenas do body (conteúdo total da página)
imagem.screenshot('imagem_capturada.png')

# Carrega e decodifica texto
resultado = reader.readtext('imagem_capturada.png', detail=0)  
# detail=0 retorna só as strings

# Junta tudo num único texto
texto_extraido = ' '.join(resultado).lower()

print (texto_extraido)

palavras_chave = "autoatendimento", "monitor", "senha"



if any(p in texto_extraido for p in palavras_chave):
    if "autoatendimento" in texto_extraido:
        print("Palavras-chave encontradas na imagem.")
        container = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.CLASS_NAME, "m-container-autoatendimento"))
        )
        actions.context_click(container).perform()
        time.sleep(1)
        actions.send_keys(Keys.ENTER).perform()
    if "senha" in texto_extraido:
        if "monitor" in texto_extraido:
            dropdown = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//span[contains(text(),'Monitor')]"))
            )
            dropdown.click()
    ...
    print("Palavras-chave não encontradas na imagem.")
